...
export default function KuponClient({ kuponi: initialKuponi }) {
  ...
  const handleAddKupon = async () => {
    setLoading(true)
    try {
      const response = await axios.post("https://alatinidza.rs/api/kupon", newKupon);
  ...
  const handleEditKupon = async () => {
    ...
    if (editKupon) {
      try {
        const response = await axios.put(`https://alatinidza.rs/api/kupon/${editKupon.id}`, editKupon);
  ...
  const handleDeleteKupon = async (id) => {
    setLoading(true)
    try {
      await axios.delete(`https://alatinidza.rs/api/kupon/${id}`);
